...
// 手机验证码gennetor函数
const phoneVerCodeGennetor = function* ({ phone }, res) {
    ...
    }).catch(error => console.error(`redis读取当前手机号验证码是否存在失败！${error}`));
    ...
    return redisSet({
      ...
      .catch(error => console.error(`redis写入当前手机号请求的验证码失败！${error}`));
};
...
exports.getPhoneVerCode = ({ query: body }, res) => {
    ...
    // 执行手机验证码gennetor函数
    const curd = phoneVerCodeGennetor(params, res);
